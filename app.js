...
const defaultState = {
  ...
  filters: {
    ...
    bugs: { caught: "all", place: "", name: "" }, // 条件フィルタ削除
    sea:  { caught: "all", name: "" }             // 海の幸：場所フィルタ削除
  },
  marks: {},
  tab: "fish"
};
...
function monthsToRanges(months){
  ...
  for (let i=1;i<a.length;i++){
    ...
    ranges.push([s,p]); s=m; p=m;
  }
  ranges.push([s,p]);
  return ranges.map(([x,y])=> x===y ? `${x}月` : `${x}-${y}月`).join("、");
}
...
function renderList(kind, items){
  ...
  const title = (kind==="fish") ? "魚" : (kind==="bugs" ? "虫" : "海の幸");
  ...
  const monthOpts = Array.from({length:12},(_,i)=>i+1).map(m=>`<option value="${m}" ${s.manualMonth===m?"selected":""}>${m}月</option>`).join("");
  const dayOpts = Array.from({length:maxDay},(_,i)=>i+1).map(d=>`<option value="${d}" ${s.manualDay===d?"selected":""}>${d}日</option>`).join("");
  const curHour = clampInt(String(s.manualTime||"").split(":")[0], 0, 23, (new Date()).getHours());
  const hourOpts = Array.from({length:24},(_,i)=>i).map(h=>`<option value="${h}" ${curHour===h?"selected":""}>${h}時</option>`).join("");
  const manualDisabled = (s.nowMode !== "manual");

  const placeOptions = buildOptions(items, "place");
  const condOptions = (kind==="bugs") ? buildOptions(items, "condition") : [];
    ...
    <div class="card">
      <div class="row" style="justify-content:space-between;">
        <div>
          ...
          <div class="small">${hemiLabel} / 日時判定（${s.nowMode==="auto"?`自動：${whenAuto}`:`手動：${whenManual}${(s.manualAnytime) ? " / すべての時間" : ""}` }）</div>
        </div>
      ...
      <div class="sectionGrid">
        ...
        <div class="fitem">
          <div class="row manualRow">
            ...
            <label class="row anytimeLabel" style="gap:6px;"><input type="checkbox" id="${kind}-set-anytime" ${s.manualAnytime?"checked":""} ${manualDisabled?"disabled":""}/> <span class="inlineLabel">すべての時間</span></label>
          </div>
        </div>
  ...
  for (const it of filtered) {
    ...
    const monthsStr = monthsToRanges((it.months && it.months[s.hemisphere]) || []);
    const timeLabel = (it.time && it.time.label) ? String(it.time.label).replaceAll(" ", "").replaceAll("　","") : "";
  ...
  const rerender = ()=>{
    // 入力中に再描画しても1文字で止まらないように、フォーカスとカーソル位置を保持
  ...
  (document.querySelector(`#${kind}-set-day`) || {addEventListener:()=>{}}).addEventListener("change",(e)=>{
    const dmax = daysInMonth(state.settings.manualMonth);
    state.settings.manualDay = clampInt(e.target.value,1,dmax,1);
    rerender();
  });
  (document.querySelector(`#${kind}-set-time`) || {addEventListener:()=>{}}).addEventListener("change",(e)=>{ state.settings.manualTime = e.target.value; rerender(); });
  ...
  const rerenderDebounced = (() => {
    ...
    const tick = () => {
      // IME変換中は再描画しない（スペースで確定候補を選べるようにする）
...
async function render(){
  ...
  try{
    ...
    // Show a minimal error block in the view
    const view = state.currentView || "fish";
...
(async ()=>{
  // service worker: works on https or localhost only (safe to ignore failure)
